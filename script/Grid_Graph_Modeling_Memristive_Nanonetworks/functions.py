...
####################    -    GRAPH DEFINITION     -    ########################
def define_grid_graph(rows, cols):
...
def mod_voltage_node_analysis(G, V_list, src_list, gnd_list):
    ...
    if len(groundnode_list) != 0:
        ...
        # solve X matrix from Yx = z (SPARSE SOLUTION)
        # matY = csr_matrix(matY)
        # invmatY = np.linalg.pinv(matY.toarray())  # inverse of matY
        
        # solve X matrix from Yx = z (NON-SPARSE SOLUTION)
        ...
        raise Exception('Error: "gnd_list" is empty.')
...
def calculate_network_resistance(H, sourcenode, groundnode):
