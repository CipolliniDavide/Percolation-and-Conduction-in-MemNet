...
def plot_S6(hor=True):
    ...
    if len(Gnw_list)==8:
        ...
        axes = [[ax1a, ax1b], [ax2a, ax2b], [ax3a, ax3b], [ax4a, ax4b], [ax5a, ax5b], [ax6a, ax6b],
                [ax7a, ax7b]]
    ...
    for i, (gnw, r, [axbis1, axbis2], c) in enumerate(zip(Gnw_list, r_l, axes, colors)):
        ...
        a = np.where(dgdv < 1e-12)[0]
        if len(a) > 0:
            ind_to_remove = np.concatenate((a, [k + 1 for k in a]))
            ind_to_keep = np.ones(len(dgdv), dtype=bool)
            ind_to_keep[ind_to_remove] = False
        # # v_eq = v_eq[ind_to_keep]
        # # gnw = gnw[ind_to_keep]
        dgdv = dgdv[ind_to_keep]
        xfirst = xfirst[ind_to_keep]

        dgdvdv, xsec = take_1rst_derivative(x=xfirst, y=dgdv)
        # Find the change of sign of dgdvdv:
        peacks = signal.find_peaks(x=dgdvdv, distance=20)[0][:2]
        ...
        if (r == 2) or (r==4):
            ...
            axbis1.semilogy(v_eq, gnw, marker=".", markersize=3.8, linewidth=5,
                        label='{:s}'.format(ratio_lab[ratio_list.index(r)]), c=c)
        # axbis2.semilogy(xfirst, dgdv, marker=".", markersize=3.8, linewidth=5,
        #             label='{:s}'.format(ratio_lab[ratio_list.index(r)]), c=c)
        axbis2.semilogy(xsec, dgdvdv, marker=".", markersize=3.8, linewidth=5, # alpha=.6,
                        label='{:s}'.format(ratio_lab[ratio_list.index(r)]), c=c)

        # for ind, col in zip(np.argsort(-dgdvdv), ['red', 'green']):
...
def plot_S6_entropy(v_eq, ent_list):
    ...
    if len(Gnw_list) == 8:
        ...
        ax3a = fig.add_subplot(gs[6:7, 0])
        ax3b = fig.add_subplot(gs[7:8, 0])
        axes = [[ax0a, ax0b], [ax1a, ax1b], [ax2a, ax2b], [ax3a, ax3b], [ax4a, ax4b], [ax5a, ax5b], [ax6a, ax6b],
                [ax7a, ax7b]]
        colors = ['#1f77b4', '#7f7f7f', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b',
                  '#e377c2']  # '#7f7f7f',  '#bcbd22', '#17becf']
        ratio_lab = ['2', '4', '10', '100', r'$1\times10^3$', r'$1\times10^4$', r'$1\times10^5$', r'$1\times10^6$']
    ...
    for i, (g, ent, r, [axbis1, axbis2], c) in enumerate(zip(Gnw_list, ent_list, r_l, axes, colors)):
        ...
        dgdvdv, xsec = take_1rst_derivative(x=xfirst, y=dgdv)
        # Find the change of sign of dgdvdv:
        zero_of_dgdvdv = np.where(np.diff(np.sign(dgdvdv)))[0][0]
        peacks = signal.find_peaks(x=dgdvdv, distance=20)[0][:2]
        ...
        if (r == 2) or (r == 4):
            axbis1.plot(v_eq, g, marker=".", markersize=3.8, linewidth=5,
                            label='{:s}'.format(ratio_lab[ratio_list.index(r)]), c=c)
            # axbis1.plot(xsec, dgdvdv, marker=".", markersize=3.8, linewidth=5, alpha=.6,
            #                 label='{:s}'.format(ratio_lab[ratio_list.index(r)]), c='grey')

        else:
            axbis1.semilogy(v_eq, g, marker=".", markersize=3.8, linewidth=5,
                    label='{:s}'.format(ratio_lab[ratio_list.index(r)]), c=c)
            # axbis1.semilogy(xsec, dgdvdv, marker=".", markersize=3.8, linewidth=5, alpha=.6,
            #             label='{:s}'.format(ratio_lab[ratio_list.index(r)]), c='grey')
...
if __name__ == "__main__":
    ...
    ratio_list = [2, 4, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6] #, 1e7, 1e8]
    if len(ratio_list)==8:
        colors = ['#1f77b4', '#7f7f7f', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b',
                  '#e377c2']  # '#7f7f7f',  '#bcbd22', '#17becf']
    ...
    ratio_lab_bold = ['2', '4', '10', '100', r'$\mathbf{1x10^3}$', r'$\mathbf{1x10^4}$', r'$\mathbf{1x10^5}$',
                      r'$\mathbf{1x10^6}$']
    ratio_lab = ['2', '4', '10', '100', r'$1\times10^3$', r'$1\times10^4$', r'$1\times10^5$', r'$1\times10^6$']
    ...
    for key, ylabel, valfmt, name_, norm in zip(['Gnw_rec.npy', 'net_entropy.npy'],
        ...
        save_path_figures = join(root,
                                 '{:s}/L{:d}_newNorm_MorePl/Figures/p{:.2f}/'.format(args.save_path, args.linear_size,
                                                                            frac_of_mem_list[0]))
        ...
        for item in tqdm(l):
            ...
            if 'Gnw' in name_:
                ...
                ax.semilogy(v_eq, y_data, marker=".", markersize=3, linewidth=3.5, c=colors[ind_for_loop],
                ...
                # ax.plot(v_eq, y_data, marker=".", markersize=3.8, linewidth=3.5, c=colors[ind_for_loop],
                #         # label='{:.0e}'.format(item.ratio)
                #         label='{:s}'.format(ratio_lab[ratio_list.index(item.ratio)])
                #         )
    ...
    for i, ent in enumerate(ent_list):
        ax.plot(v_eq, ent, marker=".",  markersize=3, linewidth=3.5,
                # label='{:.0e}'.format(item.ratio)
                c=colors[i],
                label='{:s}'.format(ratio_lab[ratio_list.index(ratio_list[i])])
                )
    set_ticks_label(ax=ax, ax_type='y',
                    num=5,
                    data=ent_list,
    ...
    set_legend(ax=ax, title='Interaction strength\n' + label_gmax_gmin)
    plt.tight_layout()
    plt.savefig(save_path_figures + 'Entropy_vs_Voltage.svg', format='svg', dpi=1200)
    plt.show()
    plt.close()
